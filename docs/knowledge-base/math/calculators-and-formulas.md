...
Palmer's formula uses Fahrenheit and quarts/pounds throughout; because the 0.2 constant is a ratio of heat capacities (grain's specific heat relative to water's), it is dimensionless in the sense that it doesn't itself carry unit-conversion baggage — the formula's structure carries over directly to metric units (°C, liters per kilogram) as long as the water-to-grain ratio `R` is computed in liters/kg (a common metric mash-thickness unit) rather than qt/lb, since it appears only as a ratio within the equation:
```
Strike Temp (°C) = (0.2 / R(L/kg)) × (Target Mash Temp − Grain Temp) + Target Mash Temp
```
Note: some metric brewing calculators re-derive the constant slightly differently when solving the equivalent heat-balance equation directly in metric units rather than converting Palmer's imperial formula in place; because both approaches derive from the same "grain's specific heat ≈ 1/5 that of water" physical assumption, they should produce consistent results if implemented correctly, but BeerOS's verification pass should confirm its chosen metric implementation against a Fahrenheit/imperial calculation of the same physical scenario (convert inputs, compare outputs) rather than assume a metric source's constant is drop-in compatible without checking.
